...
python training\train_qwen_qlora.py `
  --dataset trilingual_clinical_conversation_questions.xlsx `
...
- `train_qwen_qlora.py` — trains the bilingual/trilingual question adapter
...
from `trilingual_clinical_conversation_questions.xlsx` (or the bilingual workbook).
The trilingual workbook supplies English/Hindi/Hinglish questions directly; the
script expands each source row into three supervised examples, 3 epochs, and saves to
...
dialogue policy. If the Hinglish column is missing, examples are generated by
Romanizing the Hindi column.
